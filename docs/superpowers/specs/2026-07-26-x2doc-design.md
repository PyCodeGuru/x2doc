...
Syndication 的 `full_text` 不包含可靠的富文本块信息，因此由独立的 `parsers/plaintext_blocks.py` 执行有损启发式解析。规则按优先级固定为：三反引号或三波浪线围栏及其内部内容转为代码块；独占一行的 `---` 转为分隔线；以 `•`、`- ` 或 `* ` 开头的连续行转为无序列表；以 `1.`、`2.` 等数字加句点，或以 `①`至`⑳` 开头的连续行转为有序列表；仅由 `**文本**` 构成的独行转为二级标题；其余连续非空行转为段落，空行结束当前块。该解析无法还原 X 未提供的原始语义，必须在代码注释和 README 中标注为“有损启发式”，规则通过输入文本到块模型的快照测试固定。

标题推导规则固定如下：Article 优先使用 DOM 提供的标题；Tweet 对展开链接并移除零宽字符后的 `full_text`，取第一个非空逻辑行，去掉纯媒体短链，折叠普通空白，再截取第一个 `。`、`！`、`？`、`.`、`!` 或 `?` 及其前文；若 80 个 Unicode 字符内没有句末标点则截断到 80 字符；移除标题末尾的中英文标点，结果为空时回退为 `tweet-<id>`。标题推导不修改正文。纯符号或纯 emoji 标题可以保留，但输出 slug 为空时仍回退为 `tweet-<id>`。
